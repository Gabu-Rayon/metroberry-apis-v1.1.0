function addNewRow(element, item = null) {
  let el = $(element),
    inventoryUrl = el.data("inventory-category-url"),
    partsUrl = el.data("parts-url"),
    tbody = el.find("tbody"),
  ...
  return (
    ...
    (row +=
      0 == i
        ? `<button type="button" class="btn btn-success my-1" onclick="addNewRow('${element}')">\n                <i class="fa fa-plus-circle"></i>\n            </button>`
        : ` <button type="button" class="btn btn-danger my-1" onclick="deleteRow(${i})">\n                <i class="fa fa-trash"></i>\n            </button>`),
    (row += "</td>"),
    (row += "</tr>"),
    tbody.append(row),
...
function deleteRow(i) {
  let row = $(`#row-${i}`);
  row.find("tbody").find("tr").length < 1
    ? (row.remove(), calculateTotal())
...
function calculateTotal(element = "#purchase-table") {
  ...
  rows.each(function (i, row) {
    let q = 0;
    (q = (row = $(row)).find(`#quantity-${i}`).val() ?? 0),
      (gt += parseFloat(q));
  }),
