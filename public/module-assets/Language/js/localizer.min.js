...
const localExpireTime = 864e5,
  getLocalizeUrl = document
    .querySelector("meta[name='get-localize']")
    .getAttribute("content");
var localizeData = null,
  localSetTime = null;
function getLocalKey() {
  return "local_" + ($("html").attr("lang") ?? "en");
}
function getLocalizeData(key = null) {
  if (((key = key ?? getLocalKey()), !localizeData)) {
  ...
  return localizeData &&
    localSetTime &&
    "object" == typeof localizeData &&
    currentTime - localSetTime < localExpireTime
    ? localizeData
    : {};
}
function refreshLocalizeData() {
  let localize = getLocalizeData();
  (localize && 0 !== Object.keys(localize).length) ||
...
function localize(key) {
  if (!key || 0 === key.length || " " === key || "" === key || null === key)
    return "";
  ...
  if (
    ...
    if (localData[formattedKey]) return localData[formattedKey];
...
refreshLocalizeData(), (window.localize = localize);
